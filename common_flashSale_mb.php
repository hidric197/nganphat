...
$sql .= " , A.product_old_price, A.product_down_price, A.product_sell_price , A.product_flash_sale_time";
...
<div id='catalog-fs' class='box-catalog'>
	...
	<div class='cat-product-highlight'>
		<div class='list-product-highlight spotlight-products'
			...
			data-pages='0' data-max='60' data-length='6' data-maxpage='10'
			data-rd='24'>
			<ul class='product-highlight-wrap' data-pagecount='10'
				data-curpos='0'>
				<?php
    while ($row = $flashsaleproduct_result->fetch_assoc()) {
        ?>
				<li class="product-highlight-item prod-item  _list_ico_6677"
					data-pid="58135">
					...
					<div class="prod-hl-thumb">
						<a href="<?=$row['prd_permalink']?>"
							title="<?=$row['product_name']?>"> <img
							src="npad/<?=$row['image_url']?>"
							srcset="npad/<?=$row['image_url']?>" width="200" height="200"
							class="thumb-list is-thumb" alt="<?=$row['image_title']?>"></a>
					</div>

					<div class="prod-hl-name">
						<a title="<?=$row['product_name']?>"
							href="<?=$row['prd_permalink']?>"><?=$row['product_name']?></a>
					</div>

					<div class="prod-hl-brand">
						<a href="/sao-thai-duong.html"></br></a>
					</div>
					<div class="product-rate">
						<span class="rating-box" title="3,0 sao"><span
							class="fa fa-star rated"></span><span class="fa fa-star rated"></span><span
							class="fa fa-star rated"></span><span class="fa fa-star"></span><span
							class="fa fa-star"></span></span><span class="amount-rate"></span>
					</div>
					<div class="product-price list-product-price">
						<span class="product-price-meta list-product-meta-price">69.000 đ</span>
					</div>
    ...
    ?>
			...
			<!-- 					<label class='arrow-right next'><i class='fa fa-angle-right'></i></label> -->
			<!-- 				</div> -->
			<!-- 			</nav> -->
		</div>
	</div>
