...
<div class="row">
	<ol class="breadcrumb">
		...
		<li class="active">Quản lý Ảnh Brand</li>
	</ol>
</div>
<!--/.row-->
<div class="row">
        ...
        foreach ($files as $file) {
            ...
            if ($handle->uploaded) {
                ...
                if ($handle->processed) {
                    ...
                    $sql = "UPDATE np_prod_brand SET image_title = ?, image_url = ? ";
                    ...
                    $message = "(<span style='color: red'>Update Ảnh cho Brand thành công !</span>)";
                } else {
...
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading">Thêm Ảnh Brand <?=$brandname?> (150 x 50) <?=$message?></div>
...
<?php
    ...
    if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'delete' && isset($_REQUEST['id']))
    {
        ...
        $sql = "SELECT image_url FROM np_prod_brand WHERE brand_id = '$id'";
        ...
        $sql = "UPDATE np_prod_brand SET image_title = '', image_url = '' WHERE brand_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $message = "(<span style='color: red'>Xóa bản ghi thành công</span>)";
    }
    $sql = "SELECT A.brand_id, A.brand_name, A.image_url FROM np_prod_brand A ";
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<tr>
					...
					<td><a href="?pcid=<?=$_REQUEST['pcid']?>&display=addnew&id=<?=$row['brand_id']?>&brname=<?=$row['brand_name']?>">Update Ảnh</a></td>
