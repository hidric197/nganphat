...
<div class="row">
	<ol class="breadcrumb">
		...
		<li class="active">Quản lý Ảnh Bộ Lọc Thương Hiệu</li>
	</ol>
</div>
<!--/.row-->
<div class="row">
        ...
        foreach ($files as $file) {
            ...
            if ($handle->uploaded) {
                ...
                if ($handle->processed) {
                    ...
                    $sql = "UPDATE np_prod_filter SET image_title = ?, image_url = ? ";
                    ...
                    $message = "(<span style='color: red'>Update Ảnh cho Bộ Lọc Thương Hiệu thành công !</span>)";
                } else {
...
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading">Thêm Ảnh Bộ Lọc Thương Hiệu <?=$brandname?> (150 x 50) <?=$message?></div>
...
<?php
    ...
    if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'delete' && isset($_REQUEST['id']))
    {
        ...
        $sql = "SELECT image_url FROM np_prod_filter WHERE prod_filter_id  = '$id'";
        ...
        $sql = "UPDATE np_prod_filter SET image_title = '', image_url = '' WHERE prod_filter_id  = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $message = "(<span style='color: red'>Xóa bản ghi thành công</span>)";
    }
    $sql = "SELECT A.prod_filter_id , A.prod_filter_name, A.image_url FROM np_prod_filter A ";
    $sql .= " WHERE A.delete_flag = '0' ORDER BY prod_filter_id DESC ";
...
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading">Danh sách Ảnh Bộ Lọc Thương Hiệu <?=$message?></div>
			<div class="panel-body">
				<table class="table">
				<tr>
					<td>No</td>
				...
				<tr>
					...
					<td><a href="?pcid=<?=$_REQUEST['pcid']?>&display=addnew&id=<?=$row['prod_filter_id']?>&brname=<?=$row['prod_filter_name']?>">Update Ảnh</a></td>
