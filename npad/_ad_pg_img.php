        ...
        foreach ($files as $file) {
            ...
            if ($handle->uploaded) {
                ...
                if ($handle->processed) {
                    ...
                    $sql = "UPDATE np_page SET image_title = ?, image_url = ? ";
...
<div class="row">
	<div class="col-lg-12">
		<div class="panel panel-default">
			<div class="panel-heading">Thêm Ảnh Bài viết " <?=$brandname?> " <?=$message?></div>
...
<?php
    ...
    if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'delete' && isset($_REQUEST['id']))
    {
        ...
        $sql = "SELECT image_url FROM np_page WHERE page_id   = '$id'";
        ...
        $sql = "UPDATE np_page SET image_title = '', image_url = '' WHERE page_id   = ?";
        $stmt = $conn->prepare($sql);
    ...
    $sql = "SELECT A.page_id  , A.page_name, A.image_url FROM np_page A ";
    $sql .= " WHERE A.delete_flag = '0' ORDER BY page_id  DESC ";
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<tr>
					...
					<td><a href="?pcid=<?=$_REQUEST['pcid']?>&display=addnew&id=<?=$row['page_id']?>&brname=<?=$row['page_name']?>">Update Ảnh</a></td>
