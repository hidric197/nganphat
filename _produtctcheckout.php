					...
					<div id="ctl00_pageBody_ajxPanel" class="checkout-info-panel">
						<div id="ctl00_pageBody_ajxProducts"
							...
							<div class="checkout-info-box product" id="cart-items">
								...
								<div class="checkout-info-title">Đơn hàng</div>
										...
										foreach ($checkout_info as $itemInfo) {
										    ...
										    if ($resultCo->num_rows > 0) {
										        while ($rowCo = $resultCo->fetch_assoc()) {
										            $price = $rowCo['product_sell_price'];
										            if (!is_numeric($price)) {
										                $price = 0;
										            }
										            $totalMoney += ($price * $itemInfo[1]);
										            
										?>
										
											<li class="product-checkout-item">
												<div class="product-checkout-item-right" style="width: calc(95% - 80px);">
						...
						<div id="ctl00_pageBody_billinfo" class="checkout-info-box buyer">
							...
							<div class="checkout-info-title">Thông tin mua hàng</div>
							<div class="checkout-info-body">
								...
								<div class="input-text-checkout-item">
									...
									<input type="text"  class="account-input" id="checkout_address" name="checkout_address"  value="<?=$checkout_address?>" />
								</div>
								<div class="input-text-checkout-item">
									<span class="title-input-left">Yêu cầu khác:</span>
									<textarea class="account-input" id="checkout_other" name="checkout_other"></textarea>
								</div>
							</div>
							<?=$message?>
							<br>
